...
function updateBannerHeight() {
  ...
  if (ccBanner && ccBanner.offsetParent !== null) {
    root.style.setProperty('--cc-banner-height', `${ccBanner.offsetHeight}px`);
  } else {
    root.style.setProperty('--cc-banner-height', '0px');
  }
}
// Gère dynamiquement la hauteur du bandeau CookieConsent pour la navbar
(function () {
  function updateBannerHeight() {
    const ccBanner = document.querySelector('.cc-window');
    const root = document.documentElement;
  ...
  // Met à jour la hauteur APRÈS l'animation d'apparition du bandeau
  ...
  // Initialisation : après l’injection du DOM du bandeau
  setTimeout(listenBannerAnimation, 40); // court délai pour laisser le temps au DOM

  // Responsive : update sur resize écran
  window.addEventListener('resize', updateBannerHeight);

  // Quand on ferme/accepte/refuse le bandeau, on attend la fin de l’animation pour réinitialiser la variable
  document.body.addEventListener('click', function (e) {
    if (e.target.closest('.cc-btn')) {
      setTimeout(updateBannerHeight, 400); // délai = durée animation bandeau
    }
  });
})();
